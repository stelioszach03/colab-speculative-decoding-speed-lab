...
The two studies use different Qwen checkpoints, prompt sets and decoding regimes. They do not provide a cross-family comparison or a controlled causal backend ablation. The measured aggregate latency ratio in Study~2 is $1.387\times$; the $1.460\times$ value concerns only the math category. Neither establishes output equivalence or a general serving speedup.
\end{abstract}
...
This unpublished exploratory report asks what latency changes appeared in two recorded Colab experiments on one A100. Independent replication has not been performed. The saved studies use a shared reporting protocol:
...
The contribution is a small recorded engineering experiment, including a negative result for assisted decoding. It is not a new algorithmic variant or a replicated serving benchmark.
...
\subsection{Within-run descriptive comparisons (Study~2)}
Both recorded speculation budgets exceed $1\times$ (1.387 and 1.325), as do both prompt-category ratios (math 1.460, summarization 1.323). These overlapping views of one 24-prompt experiment are not independent replications, confidence intervals or evidence of run-to-run stability.

\section{Discussion}
\paragraph{Study~1 slowdown.}
Assisted generation was slower in every recorded cell. Tokenizer handling and runtime overhead are possible explanations, but neither was isolated or measured with profiling. The saved measurements do not establish a cause.

\paragraph{Study~2 latency reduction.}
vLLM with EAGLE-3 had lower measured latency than its baseline in this recorded setup. There are no kernel or scheduling traces establishing which mechanism caused the difference, and the cross-study changes prevent attributing it to the serving backend alone.
...
\begin{itemize}[leftmargin=1.2em]
    ...
    \item Cross-study confounds exist: different checkpoints from one model family, prompt sets and decoding regimes.
    ...
    \item No multi-seed repeated-run error bars were computed; ablation/category views within Study~2 are descriptive rather than stability evidence.
